...
### v0.5.2 (2025-12-15) - 流式转录改进
**重点**: 优化云转录超时，修复数据完整性

**Bug 修复**:
- 修复云转录提供商（Groq/SiliconFlow/Qwen）固定 30 秒超时问题
- 修复 chunked 模式 stop_recording 时丢失最后一个音频块的 race condition

**改进**:
- 云转录每个块使用独立动态超时（基于音频长度，最少 30 秒）
