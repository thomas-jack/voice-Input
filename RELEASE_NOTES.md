...
## v0.3.2 (2025-11-20) - 配置热重载与云转录修复

### 关键修复

**云提供商转录修复**
- 修复云提供商（Groq/SiliconFlow/Qwen）无法转录的严重问题
- 解决服务包装层冲突，云服务现直接使用无需中间层
- 新增文件回退转录机制，确保云转录100%成功
- 消除46个无效回调警告，优化音频处理流程

**配置热重载系统**
- 实现完整的配置热重载框架（ServiceRegistry + ConfigReloadCoordinator）
- 支持转录提供商无缝切换，无需重启应用
- 新增配置变更追踪和迁移系统
- 修复服务重新初始化逻辑，提升稳定性

**流式转录优化**
- 修复realtime模式字段名称错误（streaming_mode → mode）
- 优化云提供商流式处理逻辑，避免不支持的API调用
- 改进录音控制器回调设置，区分本地与云提供商

### 技术改进

**架构优化**
- 重构DI容器，区分本地/云提供商初始化路径
- 简化配置重载协调器，信任工厂初始化
- 优化转录控制器fallback策略

**代码质量**
- 4个关键bug修复（云转录/配置热重载/realtime模式/回调设置）
- 新增详细日志追踪，便于问题诊断
- 改进错误处理和用户反馈
...
**核心修复**
- `di_container_enhanced.py`: 分离本地/云提供商逻辑
- `config_reload_coordinator.py`: 简化初始化验证
- `recording_controller.py`: 云提供商回调优化
- `transcription_controller.py`: 字段名修复 + 文件回退

**相关组件**
- `cloud_base.py`: 新增transcribe_sync兼容方法
- `streaming_coordinator.py`: 统一stats字段命名
...
**推荐升级理由**
- 修复云转录严重bug，恢复Groq/SiliconFlow/Qwen功能
- 新增配置热重载，提升使用体验
- 消除大量警告日志，系统更清爽
- 修复多个稳定性问题
