...
class AudioVisualizer(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None, width: int = 300, height: int = 100):
    ...
    def update_waveform(self, audio_data: np.ndarray) -> None:
        ...
        try:
            ...
            if len(audio_data) != self.buffer_size:
                # 简单的重采样
                indices = np.linspace(0, len(audio_data) - 1, self.buffer_size, dtype=int)
    ...
    def _draw_idle_state(self, painter: QPainter) -> None:
        ...
        painter.drawText(self.rect(), Qt.AlignmentFlag.AlignCenter, "Press hotkey to start recording")
    ...
    def set_colors(self, background: QColor = None, waveform: QColor = None, grid: QColor = None) -> None:
        """设置颜色主题"""
        if background:
