...
class TranscriptionService:
    ...
    def _handle_transcribe(self, task: TranscriptionTask) -> None:
        ...
        try:
            ...
            if not self._is_model_loaded:
                raise WhisperLoadError("Model not loaded. Call load_model_async() first.")
            ...
            app_logger.log_error(e, f"transcribe_task_{task.task_id}")
            ...
            if task.error_callback:
                ...
                # 如果没有error_callback,通过callback传递失败结果
                task.callback(result)
    ...
    def transcribe(self, audio_data: np.ndarray, language: Optional[str] = None) -> Dict[str, Any]:
        ...
        # 阻塞等待结果
        result_event.wait(timeout=60.0)  # 60秒超时
