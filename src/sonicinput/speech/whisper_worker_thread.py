...
class WhisperWorkerThread(QThread):
    ...
    def __init__(self, parent=None):
        ...
        app_logger.log_gui_operation("WhisperWorkerThread", "Initialized dedicated whisper worker thread")

    def set_parameters(self, whisper_engine, model_name: str, timeout_seconds: int = 300):
        """设置加载参数"""
        self.whisper_engine = whisper_engine
        self.model_name = model_name
        self.timeout_seconds = timeout_seconds

        app_logger.log_gui_operation("WhisperWorkerThread Parameters",
    ...
    def reset_dll_environment(self):
        """重置DLL环境以避免Qt干扰"""
        try:
            app_logger.log_model_loading_step("Starting DLL environment reset in worker thread")

            # 步骤1: 记录当前环境状态
            original_path = os.environ.get('PATH', '')
            app_logger.log_model_loading_step("Original PATH recorded", {
            ...
            app_logger.log_model_loading_step("DLL environment reset completed successfully")
            self.environment_reset.emit(True)

        except Exception as e:
            app_logger.log_detailed_error(e, "DLL_Environment_Reset", [
    ...
    def _reset_windows_dll_paths(self):
        ...
        try:
            ...
            try:
                ...
                app_logger.log_model_loading_step("System directories located", {
                    "system_dir": system_dir.value,
                ...
                if cuda_path and os.path.exists(cuda_path):
                    ...
                    if os.path.exists(bin_path):
                        try:
                            ...
                            app_logger.log_model_loading_step(f"Added CUDA bin to DLL search: {bin_path}")
                        except Exception as e:
                            app_logger.log_model_loading_step(f"Warning: Could not add CUDA bin path: {e}")

                    if os.path.exists(lib_path):
                        try:
                            os.add_dll_directory(lib_path)
                            app_logger.log_model_loading_step(f"Added CUDA lib to DLL search: {lib_path}")
                        except Exception as e:
                            app_logger.log_model_loading_step(f"Warning: Could not add CUDA lib path: {e}")
                    break
            ...
            if hasattr(sys, 'base_prefix'):
                ...
                if os.path.exists(python_dll_path):
                    try:
                        ...
                        app_logger.log_model_loading_step(f"Added Python DLLs to search: {python_dll_path}")
                    except Exception as e:
                        app_logger.log_model_loading_step(f"Warning: Could not add Python DLL path: {e}")
    ...
    def _clean_conflicting_env_vars(self):
        """清理可能冲突的环境变量"""
        app_logger.log_model_loading_step("Cleaning potentially conflicting environment variables")
        ...
        for var in qt_vars_to_check:
            if var in os.environ:
                ...
                app_logger.log_model_loading_step(f"Found Qt variable {var}: {original_value}")
    ...
    def _cleanup_imported_modules(self):
        """清理已导入的可能冲突的模块"""
        app_logger.log_model_loading_step("Cleaning up potentially conflicting imported modules")
        ...
        if found_modules:
            app_logger.log_model_loading_step("Found pre-loaded modules", {
                "modules": found_modules,
                "note": "These modules were already loaded before thread isolation"
            })
        else:
            app_logger.log_model_loading_step("No conflicting modules found - clean slate")
    ...
    def run(self):
        ...
        try:
            ...
            # 步骤3: 在隔离环境中加载模型
            self.progress_update.emit(f"[LOAD] Loading {self.model_name} in isolated thread...")
            ...
            try:
                ...
                app_logger.log_model_loading_step("Model loaded successfully in isolated thread", {
                ...
                app_logger.log_detailed_error(e, f"Isolated_Thread_Load_{self.model_name}", [
            ...
            app_logger.log_detailed_error(e, "WhisperWorkerThread", [
