...
class DependencyDiagnostics:
    ...
    def _generate_recommendations(self, diagnosis: Dict[str, Any]) -> List[str]:
        ...
        if not pytorch_info.get("installed", False):
            recommendations.append("[ERROR] PyTorch not installed, please install PyTorch first")
        elif not pytorch_info.get("cuda_available", False):
            recommendations.append("[WARNING] PyTorch CUDA support not available, please install CUDA version of PyTorch")

        # 检查Whisper
        whisper_info = diagnosis.get("whisper_info", {})
        if not whisper_info.get("installed", False):
            recommendations.append("[ERROR] Whisper not installed, please install openai-whisper")
        elif whisper_info.get("load_test", "").startswith("FAILED"):
            recommendations.append(f"[WARNING] Whisper load test failed: {whisper_info.get('load_test', '')}")

        # 检查CUDA环境
        cuda_info = diagnosis.get("cuda_info", {})
        if not cuda_info.get("nvcc_available", False):
            recommendations.append("[WARNING] NVCC not available, may need to install CUDA Toolkit")
        if not cuda_info.get("nvidia_smi_available", False):
            recommendations.append("[WARNING] nvidia-smi not available, may need to install/update NVIDIA drivers")
        ...
        if not python_dir_info.get("in_path", False):
            recommendations.append("[WARNING] Python directory not in PATH, may cause library loading issues")

        cuda_bin_info = important_paths.get("cuda_bin", {})
        if cuda_bin_info and not cuda_bin_info.get("in_path", False):
            recommendations.append("[WARNING] CUDA bin directory not in PATH, may cause CUDA library loading issues")

        # 生成修复建议
        if recommendations:
            recommendations.append("\n[FIX] Repair suggestions:")
            recommendations.append("1. Try reinstalling: uv sync --reinstall")
            recommendations.append("2. Install CUDA version PyTorch: uv add torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121")
            recommendations.append("3. Reinstall Whisper: uv add openai-whisper")
            recommendations.append("4. Restart application")

        return recommendations

    def save_diagnosis_report(self, diagnosis: Dict[str, Any], filepath: str = None) -> str:
        """保存诊断报告到文件"""
        if filepath is None:
            from datetime import datetime
    ...
    def print_diagnosis_summary(self, diagnosis: Dict[str, Any]) -> None:
        ...
        if recommendations:
            ...
            for rec in recommendations:
                # 移除emoji，只保留文本
                clean_rec = rec.replace("❌", "[ERROR]").replace("⚠️", "[WARNING]").replace("🔧", "[FIX]")
                print(f"  {clean_rec}")
